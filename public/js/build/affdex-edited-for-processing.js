...
affdex.Detector = function() {
  ...
  // Message passing callback for incoming messages
  // from the worker defined in affdex-worker.js
...
affdex.CameraDetector = function(element, imgW, imgH, faceMode) {
  var self = this;
  self.prototype = (affdex.Detector.bind(self))();
  ...
  var dtor = function() {
    // Called when camera detector is stopped
  ...
  //   // Promise resolve function for _startCamera
  //   cameraStream = stream;
  ...
  self.start = function() {
    if(!self.isRunning) {
      const videoEl = ctor();
      // return video element to caller in post-processing.js
      // and hand over control of starting the video and detector
      return videoEl;
